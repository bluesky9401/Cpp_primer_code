...
public:
    ...
    iter_set begin() const { return ptr_sought_position_->begin(); }
    iter_set end() const { return ptr_sought_position_->end(); }
    iter_set cbegin() const { return ptr_sought_position_->cbegin(); }
    iter_set cend() const { return ptr_sought_position_->cend(); }

    std::shared_ptr<std::vector<std::string>> getFile() { return ptr_file_; }
private:
    std::string sought_;
